...
export function ResourcesHubClient({ studentApiKey, contents }: ResourcesHubClientProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Barre de navigation par Onglets (Segmented Control Top-Tier) */}
      <div className="flex items-center gap-1.5 rounded-2xl border border-[color:var(--border)] bg-[color:var(--neutral-50)] p-1.5 shadow-2xs">
        <button
          type="button"
          onClick={() => setActiveTab("templates")}
          className={`flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs sm:text-sm font-semibold transition cursor-pointer ${
            activeTab === "templates"
        ...
        >
          <span>⚡</span>
          <span>Passerelle IA & VS Code</span>
        </button>
        ...
        >
          <span>📚</span>
          <span>Supports du Formateur {publishedCount > 0 ? `(${publishedCount})` : ""}</span>
        </button>
      </div>
